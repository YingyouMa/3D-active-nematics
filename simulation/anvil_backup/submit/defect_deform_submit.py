...
for item in tobe_list:
    ...
    if end >= max_time:
        ...
        if IF_COVER:
            ...
            if len(glob.glob(out_path+'/num.npy')) == 0:
                if_do = True
            else:
                num_old = np.load(out_path +'/num.npy')
...
def submit(k,a,n):
    ...
    Path(f'submit/log_defect_deform/{k}/{a}/{n}').mkdir(exist_ok=True, parents=True)

    with open(f"submit/log_defect_deform/{k}/{a}/{n}/submit.sh", "w") as f:
        ...
        fw('#SBATCH -p shared')
        fw(f'#SBATCH -J defect_deform_k{k}_a{a}_n{n}')
        fw(f'#SBATCH -o submit/log_defect_deform/{k}/{a}/{n}/output.txt')
        fw(f'#SBATCH -e submit/log_defect_deform/{k}/{a}/{n}/error.txt')
        fw('#SBATCH -N 1')
        ...
        fw(f'python3 defect_deform.py --k={k} --a={a} --name={n} --if_cover={IF_COVER}')

    
    subprocess.run(shlex.split(f'sbatch submit/log_defect_deform/{k}/{a}/{n}/submit.sh'))
