...
class Disclination_line:
    """
    Disclination_line class represents a disclination line.


    """
    def __init__(self, defect_indices, if_sorted=True, origin=(0,0,0), space_index_ratio=1):
        '''
        ...
        if if_sorted == False:
            defect_indices = sort_line_indices(self.defect_indices) 
        ...
        defect_coord = np.einsum('na, a -> na',
                                  defect_coord, space_index_ratio)
        
        self.defects_num = np.shape(defect_indices)[0]
        self.defect_coord = defect_coord


    def update_defects_add_mind_points(self):
        self.defect_indices_add_mid = add_mid_points_disclination(self.defect_indices)

    def update_norm(self):
        self.norm = get_plane(self.defect_indices)

    def update_center(self):
        self.center = np.average(self.defect_indices, axis=-1)

    def make_plot_line(self,
...
def defect_detect(n_origin, threshold=0, boundary_periodic=0, planes=[0,1,2], print_time=False):
    ...
    Returns
    -------
    defect_indices : numpy.ndarray, defects_num x 3
...
def find_defect_n(defect_indices, size=0):
    ...
    Parameters
    ----------
    defect_indices : array, (defects_num, 3)
                     The array that includes all the indices of defects.
                     For each defect, one of the indices should be integer and the rest should be half-integer
                     Usually defect_indices are generated by defect_defect() in this module
    ...
    Returns
    -------
    defect_n : array, (4*defects_num, 3) 
    ...
    defects_num = np.shape(defect_indices)[0]

    defect_n = np.zeros((4*defects_num,3))

    defect_n0 = np.zeros((defects_num,3))
    defect_n1 = np.zeros((defects_num,3))
    defect_n2 = np.zeros((defects_num,3))
    defect_n3 = np.zeros((defects_num,3))
    ...
    index_list = np.arange(0, 4*defects_num, 4)
...
def add_mid_points_disclination(line):
    ...
    Returns
    -------
    line_new : array, ( 2*defects_num-1 , 3 )
...
def defect_classify_into_lines(defect_indices, box_size, print_time=False, print_per=1000):
    ...
    defects_num = len(defect_indices)
    defect_left_num = defects_num # the amount of unclassified defects. Initially it is the number of all defects
    ...
    defect_indices = np.hstack([defect_indices, np.zeros((defects_num,2))])
    defect_indices[..., -2] = np.arange(defects_num)
    defect_indices[if_wall, -1] = True

    defect_sorted = np.zeros((defects_num, 5))
    defect_sorted[..., -2] = -1
    defect_sorted[..., -1] = -1
            ...
            if if_find == True:
                ...
                if print_time == True:
                    if index % print_per == 0:
                        print(f'{index}/{defects_num} = {round(index/defects_num*100,2)}%, {round(time.time()-start_here,2)}s  ',
...
def extract_lines(defect_sorted):
    ...
    line_start = np.concatenate([line_start, [line_start[-1]]])

    line_num = int(defect_sorted[-1, 3] + 1)
    lines = []
    for i in range(line_num):
        lines.append(Disclination_line(defect_sorted[ line_start[i]:line_start[i+1] , :3]))
